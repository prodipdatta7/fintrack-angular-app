...
import { UserSettings, UpdateProfileRequest, ChangePasswordRequest, ExportDataRequest } from '../models/user-settings.model';
...
export class UserService {
  ...
  updateSettings(req: UserSettings): Observable<{ message: string }> {
    return this.http.put<{ message: string }>(`${this.apiUrl}/me/settings`, req).pipe(
      tap(() => this.settings.set(req))
    );
  }
  ...
  deleteAccount(confirmPassword: string): Observable<{ message: string }> {
    return this.http.post<{ message: string }>(`${this.apiUrl}/me/delete-account`, { confirmPassword });
  }
