...
describe('TransactionDetailComponent', () => {
    ...
    beforeEach(async () => {
        transactionServiceSpy = jasmine.createSpyObj(
            'TransactionService',
            ['getTransactions', 'getTransactionById', 'getTransactionEvents'],
            {
                transactions: signal([mockTx]),
            },
        );
        transactionServiceSpy.getTransactions.and.returnValue(
            of({ items: [mockTx], totalCount: 1, page: 1, pageSize: 10 } as any),
        );
        ...
        categoryServiceSpy.getCategoryById.and.returnValue(
            of({ id: 'cat-1', name: 'Employment', type: CategoryType.Income, userId: 'u1' } as any),
        );
